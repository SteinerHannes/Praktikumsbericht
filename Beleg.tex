...
\chapter{Hochschule Mittweida}
	...
	Nach eigener Einschätzung, schreibt jeder Student der HSMW ca. 5 Prüfungen pro Semester. Das bedeutet, dass im Jahr um die 60.000 Klausuren kontrolliert werden müssen. Dazu kommt, dass die Zensuren, in das Verwaltungs-Intranet der HSMW eingetragen werden müssen. Dieser Vorgang ist ebenfalls aufwändig, repetitiv und deshalb sehr belastend.
...
\chapter{Problemstellung}\label{ch:problemstellung}
	An der Kontrolle von Klausuren arbeiten zum Ende eines Semesters Hochschulmitarbeiter über Tage. Diese Aufgabe muss stets mit hoher Konzentration erledigt werden und auch bei der anschließenden Eingabe der Noten in ein Webformular des Verwaltungs-Intranets der HSMW fällt ein kognitiv belastender Navigationsaufwand an. Denn um die Zensuren einzutragen, muss für jeden Studenten zunächst die Seminargruppe aus allen Seminargruppen und anschließend der Name herausgesucht werden. Die unübersichtlichen Listen und die hohe Anzahl an Klicks sind für die Hochschulmitarbeiter äußerst belastend. Zusätzlich wird die Suche nach der Seminargruppe durch die fast gleich lautenden Bezeichnungen erschwert, was beim Danebenklicken dazu führt, das man sich in der Navigation verirrt und von vorne beginnen muss.
	
	\section{Digitalisieren der Klausur-Daten}
	Für genau diesen Vorgang wird nach einer effizienteren Lösung gesucht. Die Prüfer sollen effektiv und möglichst zeitsparend diese Aufgabe verrichten, ohne dabei ihre Aufmerksamkeitsspanne zu überlasten. Dazu sollen die für das Eintragen der Note relevanten Daten auf der Klausur digitalisiert und ein digitales tabellerisches Format gebracht werden. Grund dafür ist, dass durch solch eine Tabelle das Eintragen automatisiert werden kann. Darüber hinaus empfiehlt es sich, digitale Kopien der Klausuren abzuspeichern, um sie nicht nur analog zu archivieren.
...
\chapter{Konzept der Dokumenten-Scanner-App}\label{ch:konzept}
 		...
 		\begin{enumerate}
 			\item Der Benutzer wählt ein Kontrollmechanismus aus. Zur Auswahl stehen:
 				\begin{itemize}
 					...
 					\item oder die Note aus der Gesamtpunktzahl berechnen und mit der eingetragenen vergleichen.
 				\end{itemize}
 			\item Aus den zuvor angelegten Regionen müssen nun die passenden Regionen ausgewählt werden. Abhängig von dem zuvor gewählten Kontrollmechanismus wird dem Benutzer Auskunft darüber gegeben, welche und wie viele Regionen auszuwählen sind.
 			\item Der Kontrollmechanismus kann gespeichert werden, sobald in jeder zu vergleichenden Komponente die Mindestanzahl an auszuwählenden Regionen erreicht wurde. Diese werden in der Scan-Vorlage hinterlegt und mit an den Server zum Speichern gesendet. So werden alle Benutzer zu den Scan-Vorlagen die selben Kontrollmechanismen besitzen.
 		\end{enumerate}
	...
	\section{Erweiterung des Konzepts}\label{sc:erweiterungkonzept}
		...
		Allerdings entstehen auch durch das OCR Fehler. Aus diesem Grund ist die sogenannte confidence neben jedem Ergebnis sichtbar. Diese sagt aus, wie sicher sich die Texterkennung bei dem Ergebnis ist. Jedoch kann es auch hier falsche Positiv-Ergebnisse geben. Deshalb ist auch das kein perfekter Indikator für die Richtigkeit. Deswegen gibt es, wie im \autoref{ch:konzept} \autoref{it:senden} schon beschreiben, die Möglichkeit, die Ergebnisse noch einmal zu überprüfen und zu korrigieren, bevor sie an den Server zum Abspeichern gesendet werden. Auch die Kontrollmechanismen reagieren auf die Änderungen der Ergebnisse, sodass Fehlermeldungen behoben werden können. 

%%%%%%%%%%%%%%% - PROTOTYP - %%%%%%%%%%%%%%%

\chapter{Entwicklung des ersten Prototyps}\label{ch:prototyp}
	Dieses Kapitel beschreibt einzelne Phasen der Entwicklung der iOS-App in einer ähnlichen Reihenfolge, wie das bekannte Wasserfall-Model (siehe \autoref{fig:waterfall}) über die Verwaltung der Entwicklung großer Softwaresysteme nach Winston W. Royce \cite{royce_managing_1970}.
