...
\chapter{Ausblick}
	Der aktuelle Stand der App bietet eine gute Grundlage für mögliche Weiterentwicklungen. In diesem Kapitel werden einige Ideen erläutert, wie die App und das gesamte Softwaresystem weiter entwickelt werden kann.
	
	Durch die Verwendung von Scan-Vorlagen ist es möglich, auch andere Dokumente, wie  Krankenscheine, Arbeitsverträge oder Urlaubsanträge ein zu scannen. Damit die Texterkennung noch besser wird, sollten jedoch noch weiter Regionen-Datentypen implementiert werden. Auch die Datenbank empfiehlt es zu überarbeitet, aber grundsätzlich funktioniert die Digitalisierung jetzt schon.
	
	Wie schon in früheren Kapiteln erwähnt sollen die Bilder zum Archivieren gespeichert werden. Mit den gesammelten Prüfungen könnten so ein online Klausuren-Einsichten entstehen, so dass keine Termine mit den jeweiligen Professoren oder Mitarbeitern mehr gemacht werden müssen. Ein großes Problem was jedoch dabei entsteht, ist die Möglichkeit der schnellen Verbreitung von Klausuren. Die Lehrenden müssten dann jedes Jahr bzw. Semester neue Klausuren erstellen, da die alten Klausuren weiter geleitet wurden. Aber auch dafür könnte es in Zukunft eventuell eine Lösung geben.
