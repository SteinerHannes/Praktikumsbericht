...
\chapter{Probleme und Grenzen der App - KOMMT INS FAZIT}\label{ch:grenzen}
	...
	\section{Weitere Probleme der App}
		...
		Unter der Haube: Links nicht gut gelöst. Sie haben keine Auswirkung...
		
%%%%%%%%%%%%%%% - TEMPLATES - %%%%%%%%%%%%%%%

\chapter{Verbesserung der Klausuren-Vorlage - IN AUSBLICK?}\label{ch:vorlage}
...
\chapter{Ausblick}
	Der aktuelle Stand der App bietet eine gute Grundlage für mögliche Weiterentwicklungen. In diesem Kapitel werden einige Ideen erläutert, wie die App und das gesamte Softwaresystem weiter entwickelt und verbessert werden kann.
	
	\paragraph{Texterkennung} Wie im Kapitel \ref{ch:grenzen} erwähnt unterstützten Vision und Tesseract Handschrift nicht. Jedoch wäre es möglich eigne Neuronale-Netze zu trainieren und implementieren, die diese Aufgabe übernehmen. Beispielsweise existiert ein Datensatz an handgeschriebene Ziffern \footurl{MNIST-Datensatz -}{http://yann.lecun.com/exdb/mnist/}, mit dem solch ein Netz trainiert werden könnte. 
	...
	\paragraph{Verbesserter App Workflow} Aktuell muss zu jeder Region ein Name vergeben werden. Allerdings gibt auch der Datentyp (\ref{fig:v6}) der Region in den meisten Fällen darüber Auskunft, wie die Region heißen sollte. Handelt es sich um den Vor- bzw. Nachnamen, die Matrikel-Nummer, Seminar-Gruppe oder die Note, taucht diese Region genau einmal in einer Scan-Vorlage auf. Nur der Datentyp Punkte sollte mehrmals vergeben werden können. 
	
	\paragraph{Weitere Plattformen} Neben der iOS-App bemühte sich Tobias Kallauke um die Entwicklung der App für Android-Geräte. Allerdings könnte die Software auf noch mehr Plattformen Anwendung finden. Neben einem Programm für Linux, MacOS und Windows ist einen Web-Plattform auch erdenklich. Das Erstellen der Bilder kann über Scan- oder Kopiergeräte erfolgen, oder 
