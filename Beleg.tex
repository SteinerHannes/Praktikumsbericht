...
\chapter{Anforderungen}\label{ch:anforderungen}
	...
	Eine Scan-Vorlagen funktioniert ähnliche, wie eine Schablone. Diese wird beispielsweise auf das Deckblatt gelegt, sodass nur noch die wichtigen Informationen, die digitalisiert werden sollen, zu sehen sind. Alles andere, was nicht von Interesse ist, wird von der Schablone überdeckt. In der Abbildung \ref{fig:schablone1} ist dieses Konzept schemahaft zu sehen. Aus dem analogen Beispiel abgeleitet, benötigt es zwei Schritte, um eine Art digitale Schablonen anzufertigen. Zuerst muss ein Foto von der Seite aufgenommen werden und anschließend müssen die Regionen eingezeichnet werden, die digitalisiert werden sollen. Eine Scan-Vorlage ist jedoch nicht nur eine digitale Schablone sondern einer Sammlung aller digitalen Schablonen zu einem Dokument. Denn jede Seite braucht eine eigene individuelle. 
	
 	Weiter benötigen die eingezeichneten Regionen eine Bezeichnung bzw. einen Typen. Die Angabe des Typs ist wichtig, da dadurch eindeutig wird, ob es sich um die Note oder den Namen des Studenten handelt. Diese Eindeutigkeit wird nicht nur für die automatische Erstellung der Tabelle benötigt, sondern auch für die Optimierung der Texterkennung. 
...
\chapter{Konzept der Dokumenten-Scanner-App}\label{ch:konzept}
	...
	Zur Digitalisierung und Kontrolle der Daten werden Scan-Vorlagen verwendet. Diese muss vor dem Einscannen der ausgefüllten Klausuren erstellt werden, um zu gewährleisten, dass die richtigen Daten digitalisiert werden. In Abbildung \ref{fig:schema1} wird der Vorgang, des Erstellens einer Scan-Vorlage schematisch dargestellt. Dabei geht man wie folgt vor:
	\begin{itemize}
		\item Zuerst wird jede Seite der Klausur fotografiert (siehe auch Abb. \ref{fig:v3}). Dabei wird in jedem Bild das Dokument erkannt, vom Hintergrund getrennt bzw. ausgeschnitten und perfekt ausgerichtet. Zudem wird der Kontrast erhöht, sodass die Schrift leichter lesbar wird. Die Abbildung \ref{fig:klausur} resultierte aus dem Prozess. 
		
		\item Anschließend markiert man diejenigen Regionen auf jeder Seite, die zu digitalisieren und/oder kontrollieren sind. In der Abbildung \ref{fig:schema1} sind die Regionen blau hervorgehoben (siehe auch Abb. \ref{fig:v9}). Zusätzlich muss jeder Region ein Name und einer der folgenden Typen zugeordnet werden: Unbekannt, Name, Matrikelnummer, Seminargruppe, Punkte und Note (siehe auch Abb. \ref{fig:v8} und \ref{fig:v9}). Mithilfe des Typs wird die Texterkennung verbessert und falsche Ergebnisse automatisch korrigiert. Dazu später mehr.
	...
	Nach der Erstellung einer Vorlage kann das Dokument bzw. Die ausgefüllte Klausur digitalisiert werden. In Abbildung \ref{fig:schema2} wird dieser Vorgang schematisch dargestellt. Dazu fotografiert man die Seiten in derselben Reihenfolge, wie in der Scan-Vorlage. Anschließend werden die Regionen der Scan-Vorlage auf das jeweilige Bild angewendet. Bei dem Prozess entstehen Bildausschnitte, die für die Texterkennung benötigt werden. Die App digitalisiert mithilfe von OCR den Inhalt der Ausschnitte. Der Typ der Region nimmt nun Einfluss auf das Ergebnis. Durch ihn wird während der Worterkennungsphase eine Liste an vordefinierten möglichen Ergebnissen eingespeist. Diese Liste hat Vorrang vor dem sogenannten Standard-Wörterbuch und verbessert dadurch die Texterkennung. Ein Beispiel für solch eine Liste könnten, alle möglichen Noten sein, wie schon in dem Kapitel \ref{ch:anforderungen} beschrieben. Auch vorstellbar ist, dass alle Seminargruppen oder Namen von Personen, die an der Klausur teilgenommen haben, dort verwendet werden. 
